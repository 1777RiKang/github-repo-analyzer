...
# ── Page: Single User Analysis ───────────────────────────────
    ...
    if analyze_btn and user.strip():
        ...
        try:
            df, stats, report, charts, chart_paths, project_health = run_single_analysis(user, resolve_token(token), max_repos)
            st.session_state["df"] = df
            st.session_state["stats"] = stats
            st.session_state["report"] = report
            st.session_state["charts"] = charts
            st.session_state["chart_paths"] = chart_paths
            st.session_state["project_health"] = project_health
