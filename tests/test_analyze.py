...
class TestProjectHealth:
    def test_returns_dict(self):
        ...
        assert "总仓库数" in health
        assert "Issues 开启" in health

    def test_count_matches(self):
        df = _make_sample_df(20)
        analyzer = GitHubAnalyzer(df)
        health = analyzer.project_health()
        assert health["总仓库数"] == len(analyzer.df)


class TestRepoCompleteness:
    def test_returns_dataframe(self):
        df = _make_sample_df(10)
        analyzer = GitHubAnalyzer(df)
        comp = analyzer.repo_completeness()
        assert not comp.empty
        assert "completeness_score" in comp.columns
        assert len(comp) <= len(analyzer.df)

    def test_score_range(self):
        df = _make_sample_df(10)
        analyzer = GitHubAnalyzer(df)
        comp = analyzer.repo_completeness()
        assert comp["completeness_score"].min() >= 0
        assert comp["completeness_score"].max() <= 5

    def test_sorted_descending(self):
        df = _make_sample_df(10)
        analyzer = GitHubAnalyzer(df)
        comp = analyzer.repo_completeness()
        scores = comp["completeness_score"].values
